...
namespace Godfall.Cosmo
{
    ...
    public class Cosmic : ModNPC
    {
        ...
        {
            DisplayName.SetDefault("Cosmic Pillar");
        ...
        public override void SetDefaults()
        {
            ...
            npc.lifeMax = 100000; // The Max HP the boss has on Normal
		...
		{
			if (Main.netMode != 1 && Main.rand.Next(3000) == 0)
			{
				NPC.NewNPC((int)npc.position.X - 50, (int)npc.position.Y, mod.NPCType("GalHead"));
			}

			if (Main.netMode != 1 && Main.rand.Next(150) == 0)
			{
				NPC.NewNPC((int)npc.position.X - 50, (int)npc.position.Y, mod.NPCType("Oddo"));
			}

			if (Main.netMode != 1 && Main.rand.Next(450) == 0)
			{
				NPC.NewNPC((int)npc.position.X - 50, (int)npc.position.Y, mod.NPCType("Astrofly"));
			}

			if (Main.netMode != 1 && Main.rand.Next(150) == 0)
			{
				NPC.NewNPC((int)npc.position.X - 50, (int)npc.position.Y, mod.NPCType("Bug"));
			}

			if (Main.netMode != 1 && Main.rand.Next(400) == 0)
			{
				NPC.NewNPC((int)npc.position.X - 50, (int)npc.position.Y, mod.NPCType("Dog"));
			}

			if (Main.netMode != 1 && Main.rand.Next(400) == 0)
			{
				NPC.NewNPC((int)npc.position.X - 50, (int)npc.position.Y, mod.NPCType("Meteor"));
			}
        ...
        public override void NPCLoot()
        {
        
            {
				...
				Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, mod.ItemType("HellBar"), Main.rand.Next(1, 1));
				Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, mod.ItemType("HellBar"), Main.rand.Next(1, 1));
				Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, mod.ItemType("Scythe"), Main.rand.Next(1, 1));
